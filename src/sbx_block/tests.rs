...
fn test_calc_rs_enabled_data_write_index_simple_cases() {
    {
        ...
        let table: [u32; 2 * (TOTAL * BURST) + (1 + PARITY)] = [
            00, 01, 06, 11, 16, 00, 02, 07, 12, 17, 00, 03, 08, 13, 18, 04, 09, 14, 19, 05, 10, 15,
            20, 21, 26, 31, 36, 22, 27, 32, 37, 23, 28, 33, 38, 24, 29, 34, 39, 25, 30, 35, 40,
        ];

        // go through data seq num
        for seq in 1..40 {
            let write_index = calc_data_block_write_index(seq, None, data_par_burst) as usize;
    ...
    }
    {
        ...
        let table: [u32; 5 * (TOTAL * BURST) + (1 + PARITY)] = [
            00, 01, 03, 00, 02, 04, 05, 07, 06, 08, 09, 11, 10, 12, 13, 15, 14, 16, 17, 19, 18, 20,
        ];
    ...
    }
    {
        ...
        let table: [u32; 1 * (TOTAL * BURST) + (1 + PARITY)] = [
            00, 01, 13, 25, 37, 49, 61, 73, 85, 97, 109, 121, 00, 02, 14, 26, 38, 50, 62, 74, 86,
            98, 110, 122, 00, 03, 15, 27, 39, 51, 63, 75, 87, 99, 111, 123, 04, 16, 28, 40, 52, 64,
            76, 88, 100, 112, 124, 05, 17, 29, 41, 53, 65, 77, 89, 101, 113, 125, 06, 18, 30, 42,
            54, 66, 78, 90, 102, 114, 126, 07, 19, 31, 43, 55, 67, 79, 91, 103, 115, 127, 08, 20,
            32, 44, 56, 68, 80, 92, 104, 116, 128, 09, 21, 33, 45, 57, 69, 81, 93, 105, 117, 129,
            10, 22, 34, 46, 58, 70, 82, 94, 106, 118, 130, 11, 23, 35, 47, 59, 71, 83, 95, 107,
            119, 131, 12, 24, 36, 48, 60, 72, 84, 96, 108, 120, 132,
        ];
...
fn test_calc_rs_enabled_seq_num_at_index_simple_cases() {
    {
        ...
        // go through the table
        for index in 0..table.len() {
            let seq_num_from_index = calc_seq_num_at_index(index as u64, None, data_par_burst);
...
fn test_sync_to_buffer_simple_cases() {
    ...
    {
        let mut block = Block::new(sbx_specs::Version::V1, &uid, BlockType::Meta);
    ...
    {
        let mut block = Block::new(sbx_specs::Version::V1, &uid, BlockType::Data);
...
fn test_get_version_simple_cases() {
    ...
    assert_eq!(
        Version::V1,
        Block::new(Version::V1, &uid, BlockType::Meta).get_version()
    );
    assert_eq!(
        Version::V1,
        Block::new(Version::V1, &uid, BlockType::Data).get_version()
    );

    assert_eq!(
        Version::V2,
        Block::new(Version::V2, &uid, BlockType::Meta).get_version()
    );
    assert_eq!(
        Version::V2,
        Block::new(Version::V2, &uid, BlockType::Data).get_version()
    );

    assert_eq!(
        Version::V3,
        Block::new(Version::V3, &uid, BlockType::Meta).get_version()
    );
    assert_eq!(
        Version::V3,
        Block::new(Version::V3, &uid, BlockType::Data).get_version()
    );

    assert_eq!(
        Version::V17,
        Block::new(Version::V17, &uid, BlockType::Meta).get_version()
    );
    assert_eq!(
        Version::V17,
        Block::new(Version::V17, &uid, BlockType::Data).get_version()
    );

    assert_eq!(
        Version::V18,
        Block::new(Version::V18, &uid, BlockType::Meta).get_version()
    );
    assert_eq!(
        Version::V18,
        Block::new(Version::V18, &uid, BlockType::Data).get_version()
    );

    assert_eq!(
        Version::V19,
        Block::new(Version::V19, &uid, BlockType::Meta).get_version()
    );
    assert_eq!(
        Version::V19,
        Block::new(Version::V19, &uid, BlockType::Data).get_version()
    );
}
...
fn test_get_uid() {
    ...
    for _ in 0..1000 {
        ...
        assert_eq!(
            uid,
            Block::new(Version::V1, &uid, BlockType::Data).get_uid()
        );
    }
}

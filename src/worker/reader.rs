...
pub fn make_reader(block_size    : usize,
                   ...
                   in_file       : &str,
                   tx_bytes      : SyncSender<Option<(usize, Box<[u8]>)>>,
    ...
    Ok(thread::spawn(move || {
        ...
        loop {
            worker_stop!(graceful_if_shutdown => tx_error, shutdown_flag [tx_bytes]);
            ...
            worker_stop!(graceful_if (len_read == 0) => tx_error, shutdown_flag [tx_bytes]);
            ...
            *counter.lock().unwrap() += len_read as u64;
            ...
            send!(back_off Some((len_read, buf)) =>

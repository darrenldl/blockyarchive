...
for ver in ${VERSIONS[*]}; do
  ...
  output=$(kcov_blkar encode --json --sbx-version $ver -f dummy dummy$ver.sbx \
...
for ver in ${VERSIONS[*]}; do
    ...
    output=$(kcov_blkar check --json --pv 2 --verbose dummy$ver.sbx 2>/dev/null)
    if [[ $(echo $output | jq -r ".error") != null ]]; then
        echo " ==> Invalid JSON"
        exit_code=1
    fi
...
for ver in ${VERSIONS[*]}; do
    ...
    output=$(kcov_blkar show --json --pv 1 dummy$ver.sbx 2>/dev/null)
    if [[ $(echo $output | jq -r ".error") != null ]]; then
        echo " ==> Invalid JSON"
        exit_code=1
    fi
...
for ver in ${VERSIONS[*]}; do
  ...
  output=$(kcov_blkar decode --json --verbose dummy$ver.sbx - 2>&1 > dummy$ver)
  if [[ $(echo $output | jq -r ".error") != null ]]; then
      echo " ==> Invalid JSON"
      exit_code=1
  fi

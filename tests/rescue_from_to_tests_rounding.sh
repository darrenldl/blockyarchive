...
output=$(./../blkar rescue --json dummy_empty_disk rescued_data --from $offset --to-inc 511)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
  echo " ==> Invalid JSON"
  exit_code=1
fi
...
rm rescued_data/DEADBEEF* &>/dev/null
output=$(./../blkar rescue --json dummy_empty_disk rescued_data --from $[offset + 512] --to-inc 512)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
    echo " ==> Invalid JSON"
    exit_code=1
fi
...
rm rescued_data/DEADBEEF* &>/dev/null
output=$(./../blkar rescue --json dummy_empty_disk rescued_data --from $offset)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
    echo " ==> Invalid JSON"
    exit_code=1
fi

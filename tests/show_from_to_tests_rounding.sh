...
output=$(./../blkar show --json --show-all dummy_empty_disk --from $offset --to-inc 511)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
  echo " ==> Invalid JSON"
  exit_code=1
fi
...
output=$(./../blkar show --json --show-all dummy_empty_disk --from $[offset + 512] --to-inc 512)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
    echo " ==> Invalid JSON"
    exit_code=1
fi
...
output=$(./../blkar show --json --show-all dummy_empty_disk --from $offset)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
    echo " ==> Invalid JSON"
    exit_code=1
fi

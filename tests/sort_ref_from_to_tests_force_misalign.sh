...
output=$(./../blkar sort -f --json --ref-from $offset --force-misalign dummy.ecsbx)
...
output=$(./../blkar sort -f --json --ref-from $offset --ref-to-inc $offset --force-misalign dummy.ecsbx)
if [[ $(echo $output | jq -r ".error") == "Error : Failed to find reference block" ]]; then
  echo -n " ==> Okay"
else
  echo -n " ==> NOT okay"
...
output=$(./../blkar sort -f --json --ref-from $[offset + 512] --force-misalign dummy.ecsbx)
if [[ $(echo $output | jq -r ".error") != "Error : Failed to find reference block" ]]; then
  echo " ==> Okay"
else
  echo " ==> NOT okay"

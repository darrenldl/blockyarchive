...
output=$(./rsbx encode --json --uid $uid1 -f dummy rescue_picky_uid1.sbx)
...
output=$(./rsbx encode --json --uid $uid2 -f dummy rescue_picky_uid2.sbx)
...
output=$(./rsbx encode --json --uid $uid3 -f dummy rescue_picky_uid3.sbx)
...
output=$(./rsbx rescue --json --only-pick-uid $uid_unused dummy_disk2 rescued_data2)
...
output=$(./rsbx rescue --json --only-pick-uid $uid1 dummy_disk2 rescued_data2)
if [[ $(echo $output | jq -r ".error") != null ]]; then
    echo " ==> Invalid JSON"
    exit_code=1
fi
...
output=$(./rsbx decode --json "rescued_data2/"$uid1 "rescued_data2/"$uid1.decoded)
if [[ $(echo $output | jq -r ".stats.fileUID") != "$uid1" ]]; then
  echo " ==> Invalid JSON"
  exit_code=1
fi

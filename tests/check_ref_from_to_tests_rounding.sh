...
output=$(./../blkar check --json --ref-from $offset dummy.ecsbx)
if [[ $(echo $output | jq -r ".error") == "null" ]]; then
  echo -n " ==> Okay"
else
  echo -n " ==> NOT okay"
...
output=$(./../blkar check --json --ref-from $offset --ref-to-inc $offset dummy.ecsbx)
...
output=$(./../blkar check --json --ref-from $[offset + 512] dummy.ecsbx)
if [[ $(echo $output | jq -r ".error") == "null" ]]; then
  echo " ==> Okay"
else
  echo " ==> NOT okay"

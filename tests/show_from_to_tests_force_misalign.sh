...
output=$(./../blkar show --json --show-all dummy_empty_disk --from $offset --to-inc 511 --force-misalign)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
  echo " ==> Invalid JSON"
  exit_code=1
fi
...
output=$(./../blkar show --json --show-all dummy_empty_disk --from $[offset + 512] --to-inc 512 --force-misalign)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
    echo " ==> Invalid JSON"
    exit_code=1
fi
...
output=$(./../blkar show --json --show-all dummy_empty_disk --from $offset --force-misalign)
if [[ $(echo $output | jq -r ".error") != "null" ]]; then
    echo " ==> Invalid JSON"
    exit_code=1
fi
